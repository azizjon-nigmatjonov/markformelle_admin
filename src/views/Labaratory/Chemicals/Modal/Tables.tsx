...
import {DetailTableLogic, TableHeadersLogic, TablesLogic, TrailTableLogic} from "./Logic";
...
export const LabModalTables = ({
    ...
    const [atisNo, setAtisNo] : any = useState({ATISNO: null, LABRECETEATISID: null});
    const [openPopUp, setOpenPoUp] = useState(null);
    const [openNewModal, setOpenNewModal] = useState < string > ("");
    const {headColumns, trailHeadColumns, detailHeadColumns} = TableHeadersLogic();
    const {trailData, refetch: refetchTrailTable} = TrailTableLogic({id: formId, idTable: idTable});
    const {data: templateData, getList} = useFetchTypeSingle({setOpenNewModal});
    const [refetchStatus, setRefetchStatus] = useState(false);

    const {detailData, refetch: refetchDetailTable} = DetailTableLogic({id: formId, idTable: idTrail});
    const refetchAll = () => {
        refetchMaterial();
        refetchTrailTable();
        refetchDetailTable();
    ...
    const {deleteFn: deleteTrailFn} = TrailFormLogic({refetchTable: refetchAll});
    const {deleteFn: deleteDetailFn} = DetailFormLogic({refetchTable: refetchDetailTable});

    useEffect(() => {
        if (open.length) 
            setUniqueID(open);
         else 
            setUniqueID("main_table_lab");
    ...
    const handleActionsMaterial = (el : {
        index: number;
    ...
    const handleKeyDown = (e : KeyboardEvent) => {
        if (e.key === "Enter") {
            const arr = bodyData ?. filter((el : any) => el.HAMADI.toLocaleLowerCase().includes(search.toLocaleLowerCase()));
    ...
    return (
        <>
            <div className="grid grid-cols-3 gap-x-2">
                <div>
                    <TableUI title="labrecetecalisma"
            ...
            <TwoRowTable title="labreceteatis"
                ...
                handleRightClick={
                    (e : any, el : {
                        ...
                        setAtisNo({ATISNO: el.ATISNO, LABRECETEATISID: el.LABRECETEATISID});
                    }
                }/>
        </div>

    <div>
        <TableUI title="labreceteurun"
            ...
            disabled={
                !trailData ?. okey ?. length && !trailData ?. okeysiz ?. length
            }/>
    </div>
</div>
...
open === "trail" || open === "material" || open === "traildetail" ? (
    <CNewMiniModal title={
            open === "material" ? "labrecetecalisma" : "labreceteatis"
        }
        handleActions={
            () => {
                setOpen("");
        ...
        open === "trail" || open === "traildetail" ? (
            <TrailForm materialData={materialData}
                onClose={
                    () => setOpen("")
                }
...
open === "detail" || open === "traildetail" ? (
    <CNewMiniModal title={"labreceteurun"}
        handleActions={
            () => {
...
openPopUp && (
    <PopupUI open={
        ...
        <AskTemplate handleActions={
            (val : string) => {
                if (val === "fetch_template") {
                    getList(`recete/${
                        formData ?. LABRECETEKODU + "." + atisNo ?. ATISNO
                    }`);
                }
                setOpenPoUp(null);
            }
...
openModal.includes("card") || openNewModal === "template_ready" ? (
    <ModalUIRecipe defaultData={templateData}
        changed={""}
        setChanged={
            () => {}
        }
