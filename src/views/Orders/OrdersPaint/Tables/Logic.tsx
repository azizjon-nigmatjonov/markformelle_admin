...
export const MaterialTableLogic = ({filterParams} : {
    filterParams: any
}) => {
    ...
    const fetchList = async (filters : any) => {
        const response = await axios.get(`${API_URL}/ormesiparisdetay/?skip=${
...
export const PaintTableLogic = ({filterParams} : {
    ...
    const fetchList = async (filters : any) => {
        const response = await axios.get(`${API_URL}/boyasiparisdetay/?skip=${
            filters.page < 2 ? 0 : (filters.page - 1) * filters.perPage
        }&limit=${
            filters.perPage
        }${
...
export const PaintVariantTableLogic = ({filterParams} : {
    filterParams: any
}) => {
    const {t} = useTranslationHook();
    const [headColumns, setHeadColumns] = useState([]);
    ...
    const fetchList = async (filters : any) => {
        const response = await axios.get(`${API_URL}/desenvaryant/?skip=${
            filters.page < 2 ? 0 : (filters.page - 1) * filters.perPage
        }&limit=${
            filters.perPage
        }${
...
export const IslemTipiTableLogic = ({filterParams} : {
    filterParams: any
}) => {
    const {t} = useTranslationHook();
    const [headColumns, setHeadColumns] = useState([]);
    ...
    const fetchList = async (filters : any) => {
        const response = await axios.get(`${API_URL}/islemtipi/?skip=${
            filters.page < 2 ? 0 : (filters.page - 1) * filters.perPage
        }&limit=${
            filters.perPage
        }${
    ...
    const {data: listData, isLoading, refetch} = useQuery([
        "GET_ISLEMTIPI_TABLE", filterParams
    ], () => fetchList(filterParams), {
        keepPreviousData: true,
        enabled: !!filterParams ?. ISLEMTIPIID
    });
    useEffect(() => {
        if (listData) {
            setBodyData(listData);
    ...
    const deleteFn = async (id : string[]) => {
        try {
            await axios.delete(`${API_URL}/islemtipi/`, {
                method: "DELETE",
                url: `${API_URL}/islemtipi/`,
                headers: {
                    "Content-Type": "application/json",
                    Accept: "application/json"
                },
