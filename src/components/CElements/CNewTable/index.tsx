...
import {areAllRowsSelectedOnPage, toggleRowGroupSelection} from "./Logic/helpers";
...
const CNewTable = ({
    ...
    disabled = false,
    defaultActions = [
        "view", "edit", "delete", "is_sellect_more"
    ],
    defaultFilters = [
        "add",
        "delete",
    ...
    const tableSettings: Record < string,
        any > = {};
    ...
    const {handleCheckbox} = TableSettingsData({filterParams, handleFilterParams});
    ...
    const pageName: any = useMemo(() => {
        const strLen = location.pathname.split("/")[2].length + location.pathname.split("/")[1].length;

        let result = location.pathname.substring(0, strLen + 2);

        if (idForTable) 
            result = result + "/" + idForTable;
    ...
    const SetFiltersFn = (obj : any) => {
        ...
        for (let key in newObj) {
            if (newObj[key]) {
                str.length ? (str += "&" + key + "=" + newObj[key]) : (str += key + "=" + newObj[key]);
            }
        }
            ...
            if (value === "sort") {
                if (search === "up") {
                    result = arr ?. sort((a : any, b : any) => {
                        ...
                        return(parseInt(bVal.replace(/\D/g, "")) - parseInt(aVal.replace(/\D/g, "")));
                    });
                }
                ...
                if (search === "down") {
                    result = arr ?. sort((a : any, b : any) => {
                        ...
                        return(parseInt(aVal.replace(/\D/g, "")) - parseInt(bVal.replace(/\D/g, "")));
                    });
                }
            }
        });
    ...
    useEffect(() => {
        ...
        const list = newBodyColumns.map((item : any, index? : any) => {
            if (animation) 
                SetAnimationEffectTime(index);
            ...
            return {
                ...
                index: filterParams.page < 2 ? index + 1 : index + filterParams.perPage * (filterParams.page - 1) + 1
            };
        }) ?? [];
        setBodySource(list);
    }, [
        ...
        const createResizableColumn = function (col : any, resizer : any, idx : number) {
            ...
            const mouseMoveHandler = function (e : any) {
                ...
                requestAnimationFrame(() => {
                    ...
                    dispatch(tableSizeAction.setTableSettings({
                        pageName,
                        colID,
                        colWidth,
                        isStiky: "ineffective",
    ...
    const calculateWidth = (colId : any, index : number) => {
        const colIdx = tableSettings ?. [pageName] ?. filter((item : any) => item ?. isStiky === true) ?. findIndex((item : any) => item ?. id === colId);

        if (index === 0) {
            return 0;
        } else if (colIdx === 0) {
            return 0;
        } else if (tableSettings ?. [pageName] ?. filter((item : any) => item ?. isStiky === true).length === 1) {
            return 0;
        } else {
            return tableSettings ?. [pageName] ?. filter((item : any) => item ?. isStiky === true) ?. slice(0, colIdx) ?. reduce((acc : any, item : any) => acc + item ?. colWidth, 0);
        }
    };
    ...
    const handleSortLogic = ({value, id, search, title} : any) => {
        ...
        if (value === "sort") {
            if (search) {
                if (sortData.find((item : any) => item.value === "sort" && item.id === id)) {
    ...
    useEffect(() => {
        if (pageOrder ?. length) {
            ...
            pageOrder.forEach((element : any) => {
                arr.push(headColumns ?. find((item : {
                    id: string
                }) => getId(item.id) === element) ?? {});
            });
            setItems(arr);
        } else {
    ...
    const tableActions = (el : any, status : string) => {
        ...
        if (status === "sellect_more") {
            if (selectedItems.includes(el.index - 1)) {
                setSelectedItems(selectedItems.filter((item : any) => item !== el.index - 1));
            } else {
                setSelectedItems([
                    ...selectedItems,
                    el.index - 1
        ...
        if (status === "translation") {
            const newArr: object[] = [];
            newHeadColumns.forEach((element : {
                id: string
            }) => {
                const obj = {
                    KEYWORD: element.id,
        ...
        if (status === "delete_multiple") {
            const selectedElements = bodySource.filter((item : {
                index: number
            }) => selectedItems.includes(item.index - 1));
            setSelectedItems(selectedItems.filter((item : number) => ! selectedElements.some((el : {
                index: number
            }) => el.index - 1 === item)));
    ...
    const handleSelectAll = () => {
        const rowIndexes = bodySource.map((item : {
            index: number
        }) => item.index - 1);
        setSelectedItems(toggleRowGroupSelection({selectedItems, currentGroup: rowIndexes}));
    };

    const getBodyCol = (column : any, item : any) => {
        return column.render ? Array.isArray(column ?. id) ? column.render(column ?. id.map((data
        : any) => item[data])) : column.render(item[column ?. id], item) : GetCurrentDate({
            date: item[column ?. id],
            type: "usually"
        });
    };
    ...
    const MemoizedTableRow = React.memo(function MemoizedTableRow({
        ...
        return (
            <TableRow key={
                ...
                tabIndex={0}>
                <td className={
                        `h-[35px] border-b border-[var(--border)] w-full ${
                            openSelect ? "flex" : "hidden"
                        } justify-center items-center`
                    }
                    style={
                        {padding: "0px !importaint"}
                }>
                    <div onClick={
                            () => tableActions(item, "sellect_more")
                        }
                        className={
                            `w-[18px] h-[18px] check rounded-[4px] border flex items-center justify-center cursor-pointer ${
                                selectedItems.includes(item.index - 1) ? "border-[var(--main)]" : "border-[var(--gray)]"
                            }`
                    }>
                        {
                        selectedItems.includes(item.index - 1) && (
                        ...
                        style={
                            {
                                ...
                                ) ?. isStiky ? "sticky" : "relative",
                                left: tableSettings ?. [pageName] ?. find(
                                    (item : any) => item ?. id === column ?. id
                                ) ?. isStiky ? calculateWidth(column ?. id, colIndex) : "0",
                                backgroundColor: "#fff",
                        ...
                        <div style={
                            ...
                            className={
                                `relative h-full flex items-center ${
                                    ...
                                    hoveredIndex === colIndex && draggingIndex !== null && hoveredIndex > draggingIndex ? "drag-hovered right" : hoveredIndex === colIndex && draggingIndex !== null && hoveredIndex < draggingIndex ? "drag-hovered left" : ""
                                }`
                        }>
                            {
                            column ?. id !== "actions" && !item.empty && getBodyCol(column, item) ? (
                                <div onDoubleClick={
                                        () => {
                                            if (column ?. click !== "custom" && column ?. id !== "actions") {
                            ...
                            defaultFilters.includes("actions") && colIndex === 0 ? (
                                <div className="relative flex items-center">
                                    ...
                                    currentIndex === rowIndex && (
                                        <div className="w-[200vw] h-[200vh] fixed left-[-50vw] top-[-50vw] z-[97]"
                                            onClick={
                                                () => {
    ...
    return (
        <div className={
            `relative cnewtable w-full rounded-t-[12px] border border-[var(--border)] ${
            ...
            <div className="h-full ">
                ...
                <div className="flex">
                    <SideFilter sideFilter={sideFilter}
                        handleClick={
                            () => setSideFilter(false)
                        }
                    ...
                    <div id="table"
                        className={
                            `flex h-full w-full overflow-scroll designed-scroll ${
                                sideFilter ? "" : "pl-5"
                            }`
                        }
                        style={
                            {
                                height: autoHeight ? autoHeight : openHeader ? "calc(100vh - 140px)" : "calc(100vh - 95px)"
                            }
                    }>
                        <div className="w-full">
                            ...
                            !loading && !bodySource ?. length ? (
                                <div className="flex justify-center items-center h-full">
                                    <img className="w-[100px]" src="/images/no-data.png" alt="no data"/>
                                </div>
                            ) : (
                                <CTableWrapper count={
                                        meta.pageCount
                                    ...
                                    dataLength={
                                        newBodyColumns ?. length
                                }>
                                    <CTableHead>
                                        <CTableRow>
                                            <td className={
                                                `sticky bg-white h-[41px] ${
                                                    openSelect ? "flex" : "hidden"
                                                ...
                                                <div onClick={
                                                    ...
                                                    {
                                                    areAllRowsSelectedOnPage(selectedItems, bodySource) && (
                                                        <CheckIcon style={
                                                            {
                                                                fill: "var(--main)",
                                                                width: 14
                                            ...
                                            newHeadColumns ?. map((column : any, index : number) => (
                                                <CTableHeadCell id={
                                                    ...
                                                    key={
                                                        column ?. innerId || column ?. id + index
                                                    }
                                                    style={
                                                        {
                                                            minWidth: tableSize ?. [pageName] ?. [column ?. id] ? tableSize ?. [pageName] ?. [column ?. id] : column ?. width ? column.width : "auto",
                                                            width: tableSize ?. [pageName] ?. [column ?. id] ? tableSize ?. [pageName] ?. [column ?. id] : column ?. width ? column.width : "auto",
                                                            position: tableSettings ?. [pageName] ?. find(
                                                                (item : any) => item ?. id === column ?. id
                                                            ) ?. isStiky ? "sticky" : "relative",
                                                            left: tableSettings ?. [pageName] ?. find(
                                                                (item : any) => item ?. id === column ?. id
                                                            ) ?. isStiky ? calculateWidth(column ?. id, index) : "0",
                                                            backgroundColor: "#fff",
                                                            zIndex: tableSettings ?. [pageName] ?. find(
                                                                (item : any) => item ?. id === column ?. id
                                                            ) ?. isStiky ? "1" : ""
                                                    ...
                                                    <div draggable={true}
                                                        ...
                                                        className={
                                                            `w-full group flex items-center min-h-[40px] px-2 flex-nowrap cursor-pointer hover:bg-[var(--border)] ${
                                                                column ?. id === "index" ? "justify-center" : "justify-between"
                                                            } ${
                                                                draggingIndex === index ? "drag-and-drop" : ""
                                                            } ${
                                                                hoveredIndex === index && hoveredIndex > draggingIndex ? "drag-hovered right" : hoveredIndex === index && hoveredIndex < draggingIndex ? "drag-hovered left" : ""
                                                            }`
                                                        }
                                                        style={
                                                            {
                                                                color: sortData ?. find(
                                                                    (item : any) => item.id === column.id
                                                                ) ? "var(--primary)" : draggingIndex === index ? "var(--primary)" : "",
                                                                textAlign: !column ?. filter ? "left" : "left",
                                                                backgroundColor: currentFilter === index ? "var(--primary50)" : ""
                                                            }
                                                    }>
                                                        <div className={
                                                            `w-full min-h-[40px] flex items-center whitespace-nowrap cursor-move ${
                                                                disabled ? "text-[var(--gray)]" : ""
                                                            }`
                                                        }>
                                                            {
                                                            column.renderHead ? Array.isArray(column.renderHead) ? column.renderHead(column.renderHead.map((data
                                                            : any) => column[data])) : column.renderHead() : column.id === "index" ? "№" : t(column ?. title) === "" ? column ?. title : t(column ?. title)
                                                        } </div>

                                                        {
                                                        column.id !== "multiple" && column.id !== "index" && !column.id.includes("actions") ? (
                                                            <TableFilter colId={
                                                                    column ?. id ?? currentFilter
                                                                }
                                                                sortData={sortData}
                                                                searchedElements={searchedElements}
                                                                handleSortLogic={
                                                                    (val : any) => handleSortLogic({
                                                                        ...val,
                                                                        title: column ?. title
                                                                    })
                                                                }
                                                                ...
                                                                searchDebounce={
                                                                    (val : any, val2 : any) => searchDebounce(val, val2)
                                                                }
                                                                disabled={disabled}
                                                                handleClick={
                                    ...
                                    <CTableBody loader={false}
                                        columnscount={
                                            newHeadColumns ?. length
                                        }
                                        rowsCount={
                                            filterParams.perPage
                                        }
                                        ...
                                        {
                                        bodySource ?. length ? (bodySource ?. map((item
                                        : any, rowIndex
                                        : any) => (
                                            <MemoizedTableRow key={
                                                    item.index
                                                }
                                                item={item}
                ...
                {
                !disablePagination ? (
                    <CPagination filterParams={filterParams}
                        count={
                            meta.pageCount
                        }
                        totalCount={
